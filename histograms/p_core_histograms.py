"""Full histogram set for a P-cores-only profiling run.

Usage:
    python3 p_core_histograms.py <system_metrics_*.csv> [-o OUT_DIR]
                                 [--max_freq 4000] [--p_cores 0-15]
                                 [--plots freq_count freq_average ...] [--show]

Output PNGs are prefixed `p_core*` / `p_cores*` so they do not collide with the
E-core run produced by e_core_histograms.py.
...
def plot_frequency_count(df, opts):
    ...
    save(opts, 'p_core_frequency_count_histogram.png')
...
def plot_frequency_average(df, opts):
    ...
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tight_layout()
    save(opts, 'p_core_frequency_average_histogram.png')
...
def plot_average_usage(df, opts):
    ...
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tight_layout()
    save(opts, 'p_cores_active_average_usage_histogram.png')
...
def plot_active_time(df, opts):
    ...
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tight_layout()
    save(opts, 'p_cores_active_histogram.png')
...
def plot_p_and_e_usage(df, opts):
    ...
    plt.legend()
    plt.tight_layout()
    save(opts, 'p_cores_active_average_usage_p_and_e_core_histogram.png')
...
def plot_each_core_frequency(df, opts):
    ...
    plt.tight_layout()
    save(opts, 'p_each_core_frequency_histogram.png')
